...
Translate ZCTA-level poverty counts to PUMAs.
[`crosswalk_data()`](https://ui-research.github.io/crosswalk/reference/crosswalk_data.md)
fetches the needed crosswalk (here from Geocorr, population-weighted)
and applies it; the `count_` prefix tells it to treat the variable as a
...
| Function | Purpose |
|----|----|
| [`get_crosswalk()`](https://ui-research.github.io/crosswalk/reference/get_crosswalk.md) | Fetch crosswalk(s), chaining steps automatically when geography and year both change |
| [`crosswalk_data()`](https://ui-research.github.io/crosswalk/reference/crosswalk_data.md) | Apply crosswalk(s) to a dataset, interpolating count and non-count variables |
| [`get_available_crosswalks()`](https://ui-research.github.io/crosswalk/reference/get_available_crosswalks.md) | List every supported geography/year combination and its source |
| [`list_nhgis_crosswalks()`](https://ui-research.github.io/crosswalk/reference/list_nhgis_crosswalks.md) | List the NHGIS (cross-decade) crosswalks specifically |
...
Pass a directory as the `cache` argument to
[`get_crosswalk()`](https://ui-research.github.io/crosswalk/reference/get_crosswalk.md)
or
[`crosswalk_data()`](https://ui-research.github.io/crosswalk/reference/crosswalk_data.md)
and each fetched crosswalk is saved there as a CSV; later calls with the
same parameters read from disk instead of re-downloading. Cached files
never expire – delete a file to force a re-download.
