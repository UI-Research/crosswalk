...
-   **Programmatic access**: No manual downloads from web interfaces
-   **Standard output**: Consistent column names across all crosswalk sources
...
```{r}
attr(crosswalked_data, "crosswalk_metadata") %>% head()
```
...
Some source year/geography -> target year/geography specifications do not have a crosswalk.
...
The intellectual credit for the underlying crosswalks belongs to the original developers. Citations are:

-   **For NHGIS:** https://www.nhgis.org/citation-and-use-nhgis-data
