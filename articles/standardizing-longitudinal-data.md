...
A common challenge with longitudinal tract-level data is that census
...
standardize all years to a consistent tract vintage.
...
The HMDA data includes a `tractid` column that contains the 11-digit
tract GEOID. Let’s prepare a subset of variables for crosswalking. We’ll
focus on count variables (total applications by race/ethnicity) which
require the `count_` prefix for automatic detection by
...
Here’s where `crosswalk` shines. We need a single crosswalk to convert
all 2010-vintage tracts to 2020-vintage tracts. The package fetches this
from IPUMS NHGIS:
...
The crosswalk contains allocation factors that specify how to distribute
values from 2010 tracts to 2020 tracts. When tract boundaries changed, a
single 2010 tract may map to multiple 2020 tracts (or vice versa), with
allocation factors summing to 1.
...
standardized to 2020 tract definitions:
